...
public:
    ...
    diagnostic_updater(this, 1.0)
  {
    ...
    // Declare early — must exist before YAML params are applied (avoid 'unknown parameter')
    this->declare_parameter("uas_msg_denylist", std::vector<int64_t>{});
...
private:
  ...
  std::unordered_set<mavlink::msgid_t> uas_msg_denylist_;
  ...
  void param_init()
  {
    ...
    auto deny_vec = this->get_parameter("uas_msg_denylist").as_integer_array();
    for (auto v : deny_vec) {
      uas_msg_denylist_.insert(static_cast<mavlink::msgid_t>(v));
    }
    if (!uas_msg_denylist_.empty()) {
      RCLCPP_INFO(get_logger(), "UAS msg filter: %zu msg IDs blocked", uas_msg_denylist_.size());
    }
