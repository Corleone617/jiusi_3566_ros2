...
static void lidar_device_callback(const lidar_device_info_t* device, bool attach)
{
	...
	std::string config_dir = "";
        ...
        if (g_send_image_mask == 1) {
            std::string mask_path = g_image_mask_abs_path;
            // Resolve relative paths against package share directory
            if (!mask_path.empty() && mask_path[0] != '/') {
                #ifdef ROS2
                    mask_path = ament_index_cpp::get_package_share_directory("odin_ros_driver") + "/" + mask_path;
                #endif
            }
            if (!mask_path.empty() && std::filesystem::exists(mask_path)) {
                int ret = lidar_set_image_mask(odinDevice, mask_path.c_str());
                if (ret == 0) {
                    #ifdef ROS2
                        RCLCPP_INFO(rclcpp::get_logger("device_cb"), "Image mask set successfully: %s", mask_path.c_str());
                    #else
                        ROS_INFO("Image mask set successfully: %s", mask_path.c_str());
                    #endif
                } else {
                    #ifdef ROS2
                        RCLCPP_ERROR(rclcpp::get_logger("device_cb"), "Failed to set image mask: %s, error: %d", mask_path.c_str(), ret);
                    #else
                        ROS_ERROR("Failed to set image mask: %s, error: %d", mask_path.c_str(), ret);
                    #endif
                }
            } else {
                #ifdef ROS2
                    RCLCPP_WARN(rclcpp::get_logger("device_cb"), "Image mask path not set or file not found: %s", mask_path.c_str());
                #else
                    ROS_WARN("Image mask path not set or file not found: %s", mask_path.c_str());
                #endif
            }
        }
        ...
        #else
            ROS_INFO("Stream config: RGB=%d, RGB_COMP=%d, IMU=%d, ODOM=%d, DTOF=%d, CLOUD_SLAM=%d",
                g_sendrgb, g_sendrgb_compressed, g_sendimu, g_sendodom, g_senddtof, g_sendcloudslam);
        #endif
