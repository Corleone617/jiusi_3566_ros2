...
OLD_PREFIX="/home/orangepi/jiusi_625/install"

# --- 0. 修复 install/ 中所有硬编码路径（适配不同设备/用户名/路径） ---
echo "[0/8] 修复 install/ 路径为当前部署位置 ($INSTALL_DIR) ..."
HARDCODED_COUNT=$(grep -rlF "$OLD_PREFIX" "$INSTALL_DIR" --include="*.sh" --include="*.bash" --include="*.cmake" 2>/dev/null | wc -l)
if [ "$HARDCODED_COUNT" -gt 0 ]; then
    ...
    echo "  已修正 $HARDCODED_COUNT 个文件 ✓"
else
    echo "  无需修正（已是最新路径或未发现硬编码）"
fi
